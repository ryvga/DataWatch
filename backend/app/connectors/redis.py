...
class RedisConnector(BaseConnector):
    ...
    def _scan_limits(self) -> tuple[int, int]:
        try:
            ...
            raise ConnectorConfigurationError(
                "Redis scan limits must be integers."
            ) from exc
    ...
    def _get_client(self):
        if self._client is None:
            ...
            if tls_mode not in {"verify_identity", "disabled"}:
                raise ConnectorConfigurationError(
                    "Redis tls_mode must be verify_identity or disabled."
                )
    ...
    def _validate_scope(self, schema: str, table: str) -> None:
        ...
        if schema != expected_schema or table != _KEYSPACE_TABLE:
            raise ConnectorConfigurationError(
                "Redis operations are restricted to the configured database keyspace."
            )
    ...
    async def get_table_schema(
        ...
        body = ",\n".join(f'  "{field}" NUMBER NULL' for field in sorted(fields))
        return f'CREATE KEYSPACE VIEW "{schema}"."{table}" (\n{body}\n);', fields
    ...
    async def validate_profile_config(
        ...
        if freshness_column is not None:
            raise ConnectorConfigurationError(
                "Redis keyspace freshness must use typed TTL/Stream monitors."
            )

    async def _scan_keys(self) -> tuple[list[bytes], bool, int]:
        client = self._get_client()
        max_keys, scan_count = self._scan_limits()
        cursor = 0
        rounds = 0
        seen: set[bytes] = set()
                ...
                if len(seen) >= max_keys:
                    page_truncated = any(
                        (
                            ...
                            else str(remaining).encode("utf-8")
                        )
                        not in seen
        ...
        for operation, value in zip(operations, raw):
            if operation == "hash":
                ...
                if not isinstance(value, list):
                    unavailable.update(
                        {"stream_groups", "stream_pending", "stream_lag"}
                    )
                    continue
    ...
    async def collect_native_profile(
        ...
        memory_values = [
            ...
            if metric["memory_bytes"] is not None
        ]
        ttl_expiring = sum(
            1
            for metric in base_metrics
            if metric["ttl_ms"] is not None and metric["ttl_ms"] >= 0
        )
        ...
        return {
            ...
            "profile_provenance": {
                ...
                "key_pattern_sha256": hashlib.sha256(
                    self._key_pattern().encode("utf-8")
                ).hexdigest(),
                "keys_observed": observed,
