...
ENRICHMENTS = {
    ...
    "4.4 Architecture proposée": [
        ...
        ("p", "Le profilage peut dépasser la durée acceptable d’une requête interactive. Il est donc exécuté par Celery. Redis porte la file et certains caches, tandis qu’APScheduler maintient un job par table active. Cette chaîne forme le plan d’exécution distribué du produit : l’API accepte la demande, la file découple le rythme des sources de celui des workers, puis chaque tâche persiste un résultat rejouable. Au redémarrage, les actifs en retard peuvent être remis en file. Les services restent asynchrones et les tâches synchrones utilisent des enveloppes contrôlées, ce qui évite de maintenir deux implémentations métier."),
